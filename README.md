# IBM Qiskit ile Kuantum Rastgele Sayı Üreteci (QRNG)
...
Süreç: 
1. QuantumCircuit ile istenen bit sayısında devre oluşturulur.
...
### IBM_Qiskit_2.py (Gerçek Kuantum Donanımı)
Bu dosya, oluşturulan devreyi internet üzerinden gerçek bir IBM kuantum bilgisayarına gönderir.

Süreç:
1. QiskitRuntimeService ile IBM bulut sistemine bağlanılır.
2. Backend Seçimi: least_busy metodu ile o an kuyruğu en az olan gerçek cihaz otomatik seçilir.
3. Transpile: Devre, seçilen cihazın fiziksel mimarisine göre optimize (derleme) edilir.
4. Job: İşlem bir "Job" (görev) olarak gönderilir ve sonuçlar dönene kadar kuyrukta beklenir.
